...
export async function getBookings(status, fromDate, toDate) {
  const SELECT_QUERY =
    "select BOOKING_ID, USER_ID, ROOM_ID, STATUS, BOOKING_REASON, DATE_BOOKED, FROM_BOOKING, TO_BOOKING, REMARKS from ROOM_BOOKINGS";
    ...
    connection.query(SELECT_QUERY, async (err, result) => {
      ...
      if (status && bookings) {
        bookings = bookings.filter(
          (booking) => booking.STATUS.toUpperCase() === status.toUpperCase()
        );
      }
      ...
      if (fromDate && toDate) {
        ...
        bookings = bookings.filter(
          (booking) =>
            new Date(booking.FROM_BOOKING).toString() === formattedFromDate &&
...
export async function bookARoom(
  user_id,
  room_id,
  from_booking,
  to_booking,
  ...
  const overlapBookings = bookings.filter((booking) => {
    ...
    return (
      ...
      ((newFromBooking.getTime() >= previousFromBooking.getTime() &&
        newFromBooking.getTime() < previousToBooking.getTime()) ||
        (newToBooking.getTime() > previousFromBooking.getTime() &&
          newToBooking.getTime() <= previousToBooking.getTime()) ||
        (newFromBooking.getTime() <= previousFromBooking.getTime() &&
          newToBooking.getTime() >= previousToBooking.getTime()))
    );
  });
  ...
  if (overlapBookings && overlapBookings.length > 0) {
    ...
    if (
      (previousReserverRole.ROLE_ID === "TE" &&
        presentReserverRole.ROLE_ID == "ST") ||
      (previousReserverRole.ROLE_ID === "AD" &&
        presentReserverRole.ROLE_ID == "TE")
    ) {
      earliestScheduleForRoom = await getEarliestAvailableSchedule(room_id);
      const bookedRooms = await getBookedRoomsPerSched(
        newBooking.from_booking,
  ...
  return new Promise((resolve, reject) => {
    if (earliestScheduleForRoom || availableRooms) {
      resolve({
        ...
        earliestRoomSchedule: earliestScheduleForRoom,
      });
    } else {
